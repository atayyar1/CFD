function [aE,aW,aN,aS,aC,bC] = applyBoundaryConditions(aE,aW,aN,aS,aC,bC,Gamma,XC,YC,Xe,Ye,Xw,Yw,Xn,Yn,Xs,Ys, ...
    Se,Sw,Sn,Ss,Nx,Ny,bc,Fe,Fw,Fn,Fs)
%% fixed the old function, dropped the nargin for the diffusion for now, might add later
% Fe,Fw,Fn,Fs optional — only needed for convective BCs
if nargin < 25
    Fe=[];Fw= []; Fn = [];Fs =[];
end
% West boundary
for j = 1:Ny
    ...
    switch lower(bc.west.type)
        ...
        case 'convective'
            Ff = Fw(1,j);
            if Ff < 0  % inflow (outward normal points -x, flow enters)
                 bC(1,j) = bC(1,j) - Ff * bc.west.value;
            end
            % outflow: already handled by assembleConvectionUpwind, I am
            % not sure about this, because there are values in the middle
            % of the picture in the assignment requirement, but will check
            % the results first.
        otherwise
